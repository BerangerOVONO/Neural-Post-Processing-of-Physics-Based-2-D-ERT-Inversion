##################################################################################################################################
# This is the class and train functions used for a straightforward inversion of 2D dcr data using deep convolutional networks.  ## 
...
# this will specify the os
def add_directory_to_df(data_names, directory, slice, verbose=True):
    """ add the directory to the dataset file names to access to data using pathlib """
    directory = Path(directory)
    data_paths = data_names.copy()
    for c in slice:
        current_col = data_paths.iloc[:, c]
        data_paths.iloc[:, c] = list(map(directory.__truediv__, current_col))
    # adding the indexes
    data_paths = data_paths.reset_index()
    if verbose:
        pg.info("The directory path has been add to the dataset names specifying the os.")
    return data_paths
# this will not specify the os
...
# ## convert ert vector data to numpy array with respecting investigation depth
def ertDataToMatrix(scheme, caracVect='rhoa', shape=(100, 400), interMethod='linear', fill_value=0):
    """ Convert ERT vector data to a matrix (an image:= a numpy array) """
    x = np.array(scheme['xreport'])
    y = np.array(scheme['zreport'])
    valToMat = np.array(scheme[caracVect])  # Valeur à interpoler
    xi = np.linspace(min(scheme.sensorPositions()[:,0]), max(scheme.sensorPositions()[:,0]), shape[1])#len(scheme_ws.sensorPositions()[:,0]))
    yi = np.linspace(min(y), 0, shape[0])# len(scheme_ws.sensorPositions()[:,0]))#len(np.unique(y)))
    X, Y = np.meshgrid(xi, yi)
    mat = np.flipud(np.round(griddata((x, y), valToMat, (X, Y), method=interMethod, fill_value=fill_value), 4))
    mat = mat.copy()  # to avoid the warning: "SettingWithCopyWarning: A value is trying to be set on a copy of a slice from a DataFrame"
    return mat 

# ### The dataset class
# ## Differents classes of dataset depending on your device capacity :
# - The class device_End2endImg might be the efficient choice if you can store all the dataset on your gpu device. 
#   In this case please state the option device=your gpu device. (The training will be fast)
# - And the class all_info_preInvData is computationnally expensive. This class might just be used for visualization. 
#   Otherwise the training will be slow with unnecessary loading if you use  this class.
class device_End2endImg(Dataset):
    """ Preload all the dataset in the gpu to speed up training """
    def __init__(self, df_dataPaths, data_dir=None, transform_x=None, transform_y=None, decay_col=0, device=None, learned_car='rhoa', shape=(100,400)):
        # dataset paths dictionnary
        ...
        self.x_images = torch.from_numpy(np.array(list(map(pd.read_csv, self.df_dataPaths.iloc[:, 1])))).to(dtype=torch.float)
        # learned caracteristic / measurement images
        self.data_simu = list(map(lambda f: ert.load(f), self.df_dataPaths.iloc[:, decay_col+3]))
        self.y_images = torch.from_numpy(np.array(list(map(lambda data: ertDataToMatrix(data, caracVect=learned_car, shape=shape), self.data_simu)))).to(dtype=torch.float)
        # transforms
...
# ## Dawn sampling the image in order to accerate the training by reducing the image dimensions 
# but this  will also degradate the image resolution and some spatial informations (edges, shape and interface).
class downSampleIMG(object):
    ...
    def __init__(self, inGrid=None, outGrid=None, out_nx=None, out_nz=None, nan=999999.9, to_tensor=False):
        self.inGrid = inGrid
...
class normRes(object):
    ...
    def __init__(self, maxDatasetRes=1000.0, logScale=False, regulLog=1):
...
def train_n_validation(Net, trainload, valload, epochs, init_lr, update_lr_epoch=[0.5, 0.9], print_freq=50, n_batches_train=1, n_batches_val=1
    ...
    for epoch in range(epochs):
        ...
        Net.train()
        running_loss = 0.0
        for i, (y, x, info_linked_id) in enumerate(trainload):
        ...
        Net.eval()
        with torch.no_grad():
            for y, x, info_linked_id in valload:
                #y, yhat = y.to(device, non_blocking=True), yhat.to(device, non_blocking=True) # use this if you can't store the all dataset in the gpu device
                y_pred = Net(x)
                loss = criterion_val(y_pred, y)
                running_val_loss += loss.item()
            running_val_loss/=n_batches_val
            Loss_val.append(running_val_loss)
...
def test_rsme(Net, testload, table_file=None, dataset_type='Multi-Layers', dataset_size=1000, config='Wenner-Schlumberger'
    ...
    nb_batches = 0
    with torch.no_grad():
        for i, (y, x, info_linked_id) in enumerate(testload):
            if device:
                y, x = y.to(device, non_blocking=True), x.to(device, non_blocking=True)
            y_pred = Net(x)
