######################################################################################################################################
# In this script we will be directly inverting drc data using deep convolutional neural networks based on U-Net architecture.       ##
# Please check the file CNNs_module.py for network implementation details and The file End2end_functions.py for dataset classes     ##
# , transforms, train and evaluation functions.                                                                                     ##
#                                                                                                                                   ##
...
from End2end_functions import *


# ## dataset path
directory = "/gpfs/projets/deepres/Neural_Networks"
csv_file = 'index_preInvDCR_paths.csv'
csv_path = os.path.join(directory, csv_file)

## If the csv file already contains the directory path
data_paths = pd.read_csv(csv_file)
## adding the directory path to the dataset name if they are not already there
#data_names = pd.read_csv(csv_path) # if the csv file does not contain the directory path, we will add it to the dataset names
#data_directory = "/gpfs/projets/deepres/Data/preInv_DCR_T0_WSnDD_saphir" # put here the directory path to your dataset 
#data_paths = add_directory_to_df_os(data_names, data_directory, slice=[0,1,2,3,4,5]) # adding the directory path to the dataset names
data_paths = data_paths.reset_index()
# checking print
print("Total indexes in the dataset: ",data_paths.shape[0])
print(data_paths)

# ## Initial dataset grid used to resize data images for training
...
out_nz = 101 # number of vertical cells (pixels) in the grid (the image becoming)
out_nx = 401 #4*nz # number of horizontal cell (pixels) in the grid (the image becoming)
...
downSample_transform = downSampleIMG(grid, outgrid, out_nx, out_nz, to_tensor=True)
normalizedOverMaxRes = normRes(maxDatasetRes=2010.0, logScale=True, regulLog=1)
...
#down_dataset = device_End2endImg(data_paths, data_dir=None, transform_x=norm_transform, transform_y=norm_transform, decay_col=0
#                                 , device=device, learned_car='rhoa', shape=(99,400))
# for Dipole-dipole
down_dataset = device_End2endImg(data_paths, data_dir=None, transform_x=norm_transform, transform_y=norm_transform, decay_col=2
                                 , device=device, learned_car='rhoa', shape=(99,400))
...
# ## save and load directory (for the trained model parameters and dataset split indices)
saving_directory = "/gpfs/projets/deepres/Trained_Models_Parameters_Comp"
...
trainload = DataLoader(train_set_down, batch_size=batch_size, shuffle=True, pin_memory=False)#, num_workers=2)
...
# saving model name
save_model_name='model_end2end_c64_nl4_b32_ALL_DD_norm2k10_log10'
if lambda_tv:
    save_model_name = save_model_name + '_TV_' + str(lambda_tv)
# loss plot title
title='Comprehensive Dataset Unet on DD-Config with LogScale'
#title='Comprehensive Dataset Unet on DD-Config with Log with TV Loss lambda= '+str(lambda_tv)
...
table_eval = test_rsme(Net_inverse, testload, table_file='Evaluation_End2end_norm2K10_comp.csv', dataset_type='Comprehensive Dataset', dataset_size=N
                       , config='Dipole-dipole',  model='Unet_e2e_dd', drop_ex_row=None, device=None)
#table_eval = test_rsme(Net_inverse, testload, table_file='Evalution_postInv_norm2K10_comp.csv', dataset_type='Comprehensive Dataset', dataset_size=N
#                       , config='Dipole-dipole',  model='Unet_dd', drop_ex_row=None, device=None)