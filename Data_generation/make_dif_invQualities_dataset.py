##############################################################################################################################
# This is the building functions behind the geological subsurfaces used to assess the effect of pre-inversion quality   ######
# on neural network performance for DC-current resistivity data in 2D without topography.                               ######  
#                                                                                                                       ######
# Objective:                                                                                                            ######
# - Create uniform subsurface with different resistivity values                                                         ######
# - Introduce in the above subsurfaces a free-form gaussian shape anomaly                                               ######
...
# - simulate data and invert it with different inversion parameterization (mesh size and regularization norms)          ######
#                                                                                                                       ######
...
norm_reguls = [False, True] # list the rebusModel for penality
...
preInv_rmses_file_name = "preInv_rmse_misfit_index_multiLayers_multiQualities_WS_part1.txt" 
...
def setSchemeInvestigationPoint(scheme, fourPointArayTypr='dd', verbose=True):
    ...
    if verbose:
        ...
        print(scheme)
    return scheme
# set configurations for mmeasurement by whether loading theme (for or from field data) or by creating them using the two upper function with the default pygimli sequences
...
def inversion_pg(data, paraDepth=25, quality=33.6, paraMaxCellSize=50, paraDX=paraDX, paraBoundary=paraBoundary, limits=None, zWeight=None, lam=20, robustData=robustData, robustModel=None):
    ...
    return mgr
# with a given mesh
def inversion_pg_inMesh(data, inMesh, limits=None, zWeight=None, lam=20, robustData=robustData, robustModel=None):
...
# saving frame liste
def set_data_saving_frames(config_names=None, noise_levels=None,  verbose=True, norm_reguls=[False, True]):
    ...
    inv_rho_file_names = [[[] for norm in norm_reguls] for _ in config_names for n in noise_levels]
    if verbose:
...
rho_file_names, rho_file_mesh_names, rhoa_datFile_names, rhoa_pandaFile_names, inv_rho_file_names, rhoa_imgFile_names = set_data_saving_frames(config_names=config_names, noise_levels=noise_levels,norm_reguls=norm_reguls, verbose=True)

# saving the frame (The csv indixes file)
def save_db_indexes_file(csv_file, rho_file_names=None, config_names=None, noise_levels=None, with_inv=True, save_img=True, norm_reguls=[False, True], verbose=True):
    ...
    for noisLevel in noise_levels:
        for i, config_name in enumerate(config_names):
            ...
            if not with_inv:
                ...
                df_index_data = pd.concat([df_index_data, df_index_data_rhoa_pg], ignore_index=False, axis=1)
                for j, norm in enumerate(norm_reguls):
                    if not norm:
                        regulName = 'L2'
                        df_index_inv_rho = pd.DataFrame({'Pg regul '+regulName+ ' pre-inversion of rhoa from '+config_name+' with '+str(noisLevel)+' % of noise': (inv_rho_file_names[i])[j]})
                        ...
                        df_index_data = pd.concat([df_index_data, df_index_inv_rho], ignore_index=False, axis=1)
                    else:
                        regulName = 'L1'
                        df_index_inv_rho = pd.DataFrame({'Pg regul '+regulName+ ' pre-inversion of rhoa from '+config_name+' with '+str(noisLevel)+' % of noise': (inv_rho_file_names[i])[j]})
                        #df_index_data = pd.concat([df_index_data, df_index_data_rhoa_pg, df_index_data_rhoa_infos, df_index_inv_rho], ignore_index=False, axis=1)
...
def save_rho_inverted(name, mgr, grid, nx, nz, save_img=True, cmin=None, cmax=None, cmap=None, logScal=True):
    ...
    #print('mgr norm info: ')
    #print(list(mgr.inv.__getstate__()))
    #print("Model norm L1: ", mgr.inv.robustModel())
    del mgr
...
def simulate_n_invert_unit(plc_o_mesh, grid, nx, nz, area, scheme, res_map, rho_name, config_name=None, mesh_on=False,noiseLevel=None, noiseAbs=5e-5, seed=None
                           ...
                           , paraDepth=None, qualities=[32, 33.4], paraMaxCellSizes=[10, 10], limits=None, zWeight=None, cmin=None, cmax=None, cmap=None, norm_reguls=[False, True]):
    """ Simulate the direct probleme, inverse the simulated data and save rhoa inv_rho for a single measurement array with a single noise level on the same subsurface """
    ...
    # inversion part
    if with_inv:
        for qp, robustModel in enumerate(norm_reguls):
            mgr = inversion_pg(data, paraDepth=paraDepth, quality=qualities[qp], paraMaxCellSize=paraMaxCellSizes[qp], limits=limits, zWeight=zWeight, robustModel=robustModel)
            if robustModel:
                rho_invert_name = 'Invert_Regul_L1_' + rhoa_name
            else:
                rho_invert_name = 'Invert_Regul_L2_' + rhoa_name
            save_rho_inverted(rho_invert_name, mgr, grid, nx, nz, save_img=True, cmin=cmin, cmax=cmax, cmap=cmap)
            (inv_rho_file_names[(config_names.index(config_name)*(len(config_name)-1)+noise_levels.index(noiseLevel))])[qp].append(rho_invert_name+'.csv')
    if save_img:
...
def simulate_accros_noisLevel(noise_levels, plc_o_mesh, grid, nx, nz, area, scheme, res_map, rho_name, config_name=None, mesh_on=False,noiseLevel=None, noiseAbs=5e-5, seed=None
                              ...
                              , inv_rho_file_names=None, paraDepth=None, qualities=[32, 33.4], paraMaxCellSizes=[10, 10], limits=None, zWeight=None, cmin=None
                              , cmax=None, cmap=None, norm_reguls=[False, True]):    
    """ Simulate data for the same measurement array with different noise levels on the same subsurface """
    for noiseLevel in noise_levels:
        simulate_n_invert_unit(plc_o_mesh, grid, nx, nz, area, scheme, res_map, rho_name, config_name=config_name, mesh_on=mesh_on, noiseLevel=noiseLevel, noiseAbs=noiseAbs, seed=seed
                               ...
                               , inv_rho_file_names=inv_rho_file_names, paraDepth=paraDepth, qualities=qualities, paraMaxCellSizes=paraMaxCellSizes, limits=limits, zWeight=zWeight
                               , cmin=cmin, cmax=cmax, cmap=cmap, norm_reguls=norm_reguls)
...
def simulate_accros_configurations(config_names, noise_levels, plc_o_mesh, grid, nx, nz, area, schemes_list, res_map, rho_name, mesh_on=False, noiseAbs=5e-5, seed=None
    ...
    for simul, config_name in enumerate(config_names):
        simulate_accros_noisLevel(noise_levels, plc_o_mesh=plc_o_mesh, grid=grid, nx=nx, nz=nz, area=area, scheme=schemes_list[simul], res_map=res_map, rho_name=rho_name, config_name=config_name
                                  ...
                                  , save_img=save_img, with_inv=with_inv, inv_rho_file_names=inv_rho_file_names, paraDepth=paraDepth, qualities=qualities, paraMaxCellSizes=paraMaxCellSizes
                                  , limits=limits, zWeight=zWeight, cmin=cmin, cmax=cmax, cmap=cmap, norm_reguls=norm_reguls)
      
# ## generate multi-layers subsurfaces, faults with and without anysotropy
def create_data_multi_layers_invQual(left, right, depth, grid, nx, nz, config_files=None, config_names=['dd','slm'], ratio=3, nb_elec=96, number_init=10, inf_res=10, sup_res=1000, area=1, cmap='jet', noiseAbs=5e-5, seed=1
                            ...
                            , paraDepth=depth, qualities=[32,33.4], paraMaxCellSizes=[20,10], limits=None, zWeight=None, anisotropy=True, inject_fluid=True, cmin=None, cmax=None, tol_lim=0.1
                            , fix_marker_area=0.1, norm_reguls=[False, True]):
    ...
    for i in range(number_init):
        ...
        if anisotropy:
            ...
            for k, anyso_mesh in enumerate(meshes):
                ...
                simulate_accros_configurations(config_names, noise_levels, anyso_mesh, grid, nx, nz, area, schemes_list, res_map=rhomaps[k], rho_name=rho_names[k], mesh_on=False, noiseAbs=noiseAbs, seed=seed
        ...
        for j in range(ratio):
            ...
            for k, rmesh in enumerate(meshes):
                ...
                simulate_accros_configurations(config_names, noise_levels, rmesh, grid, nx, nz, area, schemes_list, res_map=rhomaps[k], rho_name=rho_names[k], mesh_on=False, noiseAbs=noiseAbs, seed=seed
                                            , rhoa_datFile_names=rhoa_datFile_names, rhoa_pandaFile_names=rhoa_pandaFile_names, rhoa_imgFile_names=rhoa_imgFile_names, save_img=True, with_inv=True 
                                            , inv_rho_file_names=inv_rho_file_names, paraDepth=paraDepth, qualities=qualities, paraMaxCellSizes=paraMaxCellSizes, limits=limits, zWeight=zWeight
                                            , cmin=cmin, cmax=cmax, cmap=cmap, norm_reguls=norm_reguls)
                if anisotropy:
                    ...
                    simulate_accros_configurations(config_names, noise_levels, rmesh, grid, nx, nz, area, schemes_list, res_map=resistivity, rho_name=rho_name, mesh_on=False, noiseAbs=noiseAbs, seed=seed
                                            , rhoa_datFile_names=rhoa_datFile_names, rhoa_pandaFile_names=rhoa_pandaFile_names, rhoa_imgFile_names=rhoa_imgFile_names, save_img=True, with_inv=True 
                                            , inv_rho_file_names=inv_rho_file_names, paraDepth=paraDepth, qualities=qualities, paraMaxCellSizes=paraMaxCellSizes, limits=limits, zWeight=zWeight
                                            , cmin=cmin, cmax=cmax, cmap=cmap, norm_reguls=norm_reguls)
            
            save_db_indexes_file(csv_file=csv_file, rho_file_names=rho_file_names, config_names=config_names, noise_levels=noise_levels, with_inv=True, save_img=True, norm_reguls=norm_reguls, verbose=True)        
            ...
            if j==0:
                ...
                if inject_fluid:
                    ...
                    for k, mesh_inj in enumerate(meshes):
                        ...
                        save_rho_data(name=rho_name, mesh=mesh_inj, resistivity=res, grid=grid, nx=nx, nz=nz, resIsRhomap=False, rho_file_names=rho_file_names, rho_file_mesh_names=rho_file_mesh_names
                                    , save_img=True, cmin=cmin, cmax=cmin, cmap=cmap)
                        simulate_accros_configurations(config_names, noise_levels, mesh_inj, grid, nx, nz, area, schemes_list, res_map=res, rho_name=rho_name, mesh_on=False, noiseAbs=noiseAbs, seed=seed
                                                , rhoa_datFile_names=rhoa_datFile_names, rhoa_pandaFile_names=rhoa_pandaFile_names, rhoa_imgFile_names=rhoa_imgFile_names, save_img=True, with_inv=True 
                                                , inv_rho_file_names=inv_rho_file_names, paraDepth=paraDepth, qualities=qualities, paraMaxCellSizes=paraMaxCellSizes, limits=limits, zWeight=zWeight
                                                , cmin=cmin, cmax=cmax, cmap=cmap, norm_reguls=norm_reguls)
                ...
                for k, mesh_tecto in enumerate(meshes):
                    save_rho_data(name=rho_names[k], mesh=mesh_tecto, resistivity=rhomaps[k], grid=grid, nx=nx, nz=nz, resIsRhomap=True, rho_file_names=rho_file_names, rho_file_mesh_names=rho_file_mesh_names
                                , save_img=True, cmap=cmap)
                    simulate_accros_configurations(config_names, noise_levels, mesh_tecto, grid, nx, nz, area, schemes_list, res_map=rhomaps[k], rho_name=rho_names[k], mesh_on=False, noiseAbs=noiseAbs, seed=seed
                                                , rhoa_datFile_names=rhoa_datFile_names, rhoa_pandaFile_names=rhoa_pandaFile_names, rhoa_imgFile_names=rhoa_imgFile_names, save_img=True, with_inv=True 
                                                , inv_rho_file_names=inv_rho_file_names, paraDepth=paraDepth, qualities=qualities, paraMaxCellSizes=paraMaxCellSizes, limits=limits, zWeight=zWeight
                                                , cmin=cmin, cmax=cmax, cmap=cmap, norm_reguls=norm_reguls)
                    if anisotropy:
                        ...
                        save_rho_data(name=rho_name, mesh=mesh_tecto, resistivity=resistivity, grid=grid, nx=nx, nz=nz, resIsRhomap=False, rho_file_names=rho_file_names, rho_file_mesh_names=rho_file_mesh_names
                                    , save_img=True, cmin=cmin, cmax=cmin, cmap=cmap)
                        simulate_accros_configurations(config_names, noise_levels, mesh_tecto, grid, nx, nz, area, schemes_list, res_map=resistivity, rho_name=rho_name, mesh_on=False, noiseAbs=noiseAbs, seed=seed
                                                , rhoa_datFile_names=rhoa_datFile_names, rhoa_pandaFile_names=rhoa_pandaFile_names, rhoa_imgFile_names=rhoa_imgFile_names, save_img=True, with_inv=True 
                                                , inv_rho_file_names=inv_rho_file_names, paraDepth=paraDepth, qualities=qualities, paraMaxCellSizes=paraMaxCellSizes, limits=limits, zWeight=zWeight
                                                , cmin=cmin, cmax=cmax, cmap=cmap, norm_reguls=norm_reguls)
